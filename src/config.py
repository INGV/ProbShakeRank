...
def load_config(file):
    ...
    if str(lines[3].split( )[1]) == 'None':
        vs30file = None
    else:
        vs30file = str(lines[3].split( )[1])
    CorrelationModel = str(lines[4].split( )[1])
    CrosscorrModel = str(lines[5].split( )[1])
    vs30_clustering = lines[6].split()[1] == 'True'
    truncation_level = float(lines[7].split( )[1])
    seed = int(lines[8].split( )[1])
