...
for Nid in range(RS_samp_run):
    ...
    filename = f'./output/list_nb{Nid}_of_{RS_samp_scen}_Ev_{EV_ID}_scenbs_{timestamp}.txt'
...
timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
backup_folder = os.path.abspath(os.path.join(os.getcwd(), f'../../INPUT_FILES/BACKUP/ENSEMBLE_{EV_ID}_{timestamp}'))
